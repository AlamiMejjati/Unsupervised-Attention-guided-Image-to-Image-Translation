...
def get_outputs(inputs, network="tensorflow", skip=False):
    ...
    with tf.variable_scope("Model") as scope:
        ...
        # prob_real_a_is_real = current_discriminator(images_a, tf.ones_like(images_a), transition_rate, "d_A")
        # prob_real_b_is_real = current_discriminator(images_b, tf.ones_like(images_b), transition_rate, "d_B")
        ...
        # prob_fake_a_is_real= current_discriminator(fake_images_a, tf.ones_like(fake_images_a), transition_rate, "d_A")
        # prob_fake_b_is_real= current_discriminator(fake_images_b, tf.ones_like(fake_images_b), transition_rate, "d_B")
        ...
        # prob_fake_pool_a_is_real = current_discriminator(fake_pool_a, tf.ones_like(fake_pool_a), transition_rate, "d_A")
        # prob_fake_pool_b_is_real = current_discriminator(fake_pool_b, tf.ones_like(fake_pool_b), transition_rate, "d_B")
...
def build_generator_resnet_9blocks_bis(inputgen, mask, transition_rate, name="generator", skip=False):
    inputgen = tf.multiply(inputgen, mask)
    # mask = tf.cast(tf.greater_equal(mask, 0.1),tf.float32)
    with tf.variable_scope(name):
        f = 7
        ks = 3
        padding = "CONSTANT"
